...
class SampleSet(object):
    ...
    # julio funcion agregada
    def get_pd_csi(self):
        csi = {}
        for sample in range(len(self.csi)):
            for subcarrier in range(len(self.csi[sample])):
                if (
                    subcarrier not in nulls[self.bandwidth]
                    and subcarrier not in pilots[self.bandwidth]
                ):
                    if subcarrier in csi:
                        csi[subcarrier] = np.append(
                            csi[subcarrier],
                            self.get_csi(sample, True, True)[subcarrier],
                        )
                    else:
                        csi[subcarrier] = np.array(
                            self.get_csi(sample, True, True)[subcarrier]
                        )
    ...
    # julio funcion agregada
    def get_csi_completo(self, rm_nulls=False, rm_pilots=False):
        csi = self.csi.copy()
        if rm_nulls:
            csi[nulls[self.bandwidth]] = 0
        if rm_pilots:
...
def read_pcap(pcap_filepath, bandwidth=0, nsamples_max=0):
    ...
    # print("ancho de banda: ", bandwidth, "numero de subportadoras: ", nsub) #juliocesar
    ...
    while ptr < pcap_filesize:
        ...
        csi[nsamples * (nsub * 4) : (nsamples + 1) * (nsub * 4)] = fc[
    ...
    # print("forma de array1 : ", csi_np.shape) #juliocesar
    # print(csi_np) #juliocesar
    ...
    # print("forma de array2 : ", csi_cmplx.shape)#julio
    # print("\n  complejo -------->>>", csi_cmplx)#juliocesar

    return SampleSet((mac, seq, css, csi_cmplx), bandwidth)


if __name__ == "__main__":
    samples = read_pcap("pcap_files/testcsicapture_80_157.pcap")
